...
- 后端：FastAPI + SQLite 接口，包含商品、订单、溯源、综合经营汇总、订单变更状态和运营数据中心。
- 后端运营数据中心：访问 `http://localhost:8000/admin` 或 `http://localhost:8000/`，用于查看订单监控、履约监控、商品库存、渠道结构和数据同步状态；订单创建入口属于前端商城。
...
6. 后端运营数据中心只查看数据：订单监控、履约指标、商品库存、渠道结构和最新同步状态。
7. 页面每 5 秒检查一次订单状态版本；如果订单数据变化，页面会自动刷新。
