...
- 后端：FastAPI + SQLite 接口，包含商品、订单、溯源、综合经营汇总和订单变更状态模块。
- 汇总模块：归纳已支付订单、热销商品、履约质量、金融服务和养殖户增收等数据，用于支撑页面展示和管理分析。
- 自动刷新：后端提供订单数据版本，前端每 5 秒轻量检查一次；发现新订单或金额变化后自动刷新页面信息。
...
- `GET /api/change-state`：订单变更状态，供前端自动判断是否需要刷新
...
6. 页面每 5 秒检查一次订单状态版本；如果其他订单导致数据变化，页面会自动刷新。
